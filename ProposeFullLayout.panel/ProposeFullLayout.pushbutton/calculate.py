...
def fill_vertical_gap(region_x_start, region_x_end, gap_y_start, gap_y_end,
    ...
    while y_cursor < gap_y_end:
        ...
        while x_cursor < panel_x_end:
            ...
            # Absorb or clamp sub-min remainder
            if remaining_width > 0 and remaining_width < PANEL_WIDTH_MIN:
                ...
                if row_panels:
                    ...
                    if extended_w <= max_w_here:
                        # Absorb: widen last panel to swallow the remainder
                        last.w = extended_w
                        last.cutouts = calculate_panel_cutouts(last, all_openings)
                    else:
                        # Can't absorb. Clamp end panel to min_width, shrink previous.
                        new_last_w = round(last.w + remaining_width - PANEL_WIDTH_MIN, 8)
                        if new_last_w >= PANEL_WIDTH_MIN:
                            last.w = new_last_w
                            last.cutouts = calculate_panel_cutouts(last, all_openings)
                            end_x = round(last.x + last.w + spacing, 8)
                            end_p = Panel(end_x, y_cursor, PANEL_WIDTH_MIN, panel_h,
                                         "P{:02d}".format(panel_counter))
                            end_p.cutouts = calculate_panel_cutouts(end_p, all_openings)
                            panels.append(end_p)
                            panel_counter += 1
                        # else: both panels would go sub-min — leave last panel as-is
                break
                ...
                if remaining_wall > 0 and remaining_wall < PANEL_WIDTH_MIN:
                    ...
                    if band_panels:
                        ...
                        if extended_w <= max_width_for_band:
                            ...
                            # Can't absorb into last panel (would exceed max_width).
                            # Clamp the end panel to min_width and shrink the previous
                            # panel by the shortfall — zero overlap, zero gap.
                            #   new_last_w = last.w - (min_width - remaining_wall)
                            #   end_x      = last.x + new_last_w + spacing
                            new_last_w = round(last.w + remaining_wall - PANEL_WIDTH_MIN, 8)
                            if new_last_w >= PANEL_WIDTH_MIN:
                                last.w = new_last_w
                                last.cutouts = calculate_panel_cutouts(last, region_openings)
                                end_x = round(last.x + last.w + spacing, 8)
                                candidate = Panel(end_x, y_start, PANEL_WIDTH_MIN, band_height,
                                                 "P{:02d}".format(panel_counter))
                                candidate.cutouts = calculate_panel_cutouts(candidate, region_openings)
                                panels.append(candidate)
                                panel_counter += 1
                                print("    [FIX] Clamped end panel to min {}\" (was {:.3f}\"), "
                                      "shrunk {} to {:.3f}\"".format(
                                      PANEL_WIDTH_MIN, remaining_wall,
                                      last.name, new_last_w))
                            else:
                                # Previous panel would also go sub-min if shrunk — place as-is.
                                ...
                                panels.append(candidate)
                                panel_counter += 1
                                print("    [WARN] End panel {:.3f}\" < min — prev panel would "
                                      "also go sub-min if shrunk; placed as-is.".format(snap_w))
                    break
...
def process_all_walls(walls_rows, openings_rows, output_dir,
    ...
    for wall_row in walls_rows:
        ...
        # ---- Structural face adjustment ----------------------------------------
        # StructFaceOffset(ft): signed displacement from the location-line to the
        # exterior face of the structural (core) layer, in the outward-normal direction.
        #   negative → structural face is inward from the location line (most common:
        #              "Finish Face: Exterior" walls have finish/substrate layers between
        #              the location line and the structural substrate)
        #   zero     → location line already sits at the structural face
        # Shift wall_origin by this offset so every panel record's placement
        # coordinates land on the structural layer rather than the finish face.
        _normal_raw    = _parse_xyz(wall_row.get("Normal(unit XYZ)", ""))
        _struct_offset = safe_float(wall_row.get("StructFaceOffset(ft)", 0.0))

        # Normalise the normal vector (it should already be unit length, but guard
        # against rounding in the CSV).
        if _normal_raw:
            _nm = math.sqrt(sum(v*v for v in _normal_raw))
            _normal_xyz = tuple(v / _nm for v in _normal_raw) if _nm > 1e-9 else None
        else:
            _normal_xyz = None

        # Populate wall_normal fields (zero if no normal exported)
        _wall_geom["wall_normal_x"] = round(_normal_xyz[0], 8) if _normal_xyz else 0.0
        _wall_geom["wall_normal_y"] = round(_normal_xyz[1], 8) if _normal_xyz else 0.0
        _wall_geom["wall_normal_z"] = round(_normal_xyz[2], 8) if _normal_xyz else 0.0

        # Apply offset only when we have both a normal and a non-trivial offset
        if _normal_xyz and abs(_struct_offset) > 1e-6:
            _wall_geom["wall_origin_x"] = round(
                _wall_geom["wall_origin_x"] + _normal_xyz[0] * _struct_offset, 8)
            _wall_geom["wall_origin_y"] = round(
                _wall_geom["wall_origin_y"] + _normal_xyz[1] * _struct_offset, 8)
            _wall_geom["wall_origin_z"] = round(
                _wall_geom["wall_origin_z"] + _normal_xyz[2] * _struct_offset, 8)
            print(Ansi.CYAN + "  [STRUCT] wall_origin shifted {:.4f} ft ({:.3f} in) along normal "
                  "to structural face".format(_struct_offset, _struct_offset * 12) + Ansi.RESET)
        # -------------------------------------------------------------------------

        panel_records = process_wall(wall_id, wall_width, wall_height, openings)

        # Prefix every panel name with the facade label so panels from different
        # walls are always distinct: "N-P01", "S-T01-P02", "E-P03", etc.
        _facade_id = str(wall_row.get("FacadeId", "")).strip() or wall_id
        for _pr in panel_records:
            _pr["panel_name"] = "{}-{}".format(_facade_id, _pr["panel_name"])

        # Derive placement rotation from the wall's XY direction vector.
        # +180 deg is added so the panel's local X axis points toward the
        # exterior face.  Without it the family faces inward.
        #   ( 1,  0, 0) -> 180 deg    ( 0,  1, 0) -> 270 deg
        #   (-1,  0, 0) ->   0 deg    ( 0, -1, 0) ->  90 deg
        _rot_deg = round(
            (math.degrees(
                math.atan2(_wall_geom["wall_dir_y"], _wall_geom["wall_dir_x"])
            ) + 180.0) % 360.0,
            4
        )
        ...
        for _pr in panel_records:
            ...
            # Mirror cutout x offsets to compensate for the +180 deg flip.
            # Rotating 180 deg around Z inverts the panel's local X axis, so
            # a cutout that was x_in from the left is now at:
            #   x_in_new = panel_width - x_in - cutout_width
            # Centered cutouts are unaffected; off-center ones move to the
            # correct mirrored position instead of landing on the wrong side.
            _pw = float(_pr.get("width_in", 0))
            if _pw > 0 and _pr.get("cutouts_json"):
                try:
                    _cuts = json.loads(_pr["cutouts_json"])
                    for _c in _cuts:
                        _cw  = float(_c.get("width_in",     0))
                        _rxw = float(_c.get("raw_width_in", 0))
                        _c["x_in"]     = round(_pw - float(_c.get("x_in",     0)) - _cw,  8)
                        _c["raw_x_in"] = round(_pw - float(_c.get("raw_x_in", 0)) - _rxw, 8)
                    _pr["cutouts_json"] = json.dumps(_cuts)
                except Exception:
                    pass
