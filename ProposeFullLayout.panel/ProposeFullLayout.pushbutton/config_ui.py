...
# =============================================================================
def run_optimizer_from_folder(input_dir):
    try:
    ...
    output_dir = os.path.join(
        input_dir,
        "{0}_{1}".format(_sanitize_folder_name(config.project_name), timestamp)
    )
...
try:
    import calculator as opt
except Exception as e:
    raise Exception("Failed to import calculator.py from {0}: {1}".format(_this_dir, e))
